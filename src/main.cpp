...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+31) - GetDepthInMainChain());
}
...
{
	int64_t nSubsidy = 0.8 * COIN; // Octanox base reward

    if(nBestHeight < 2)
        nSubsidy = 0;
    if(nBestHeight == 2)
        nSubsidy = 20000000 * COIN; // Octanox premine
    if(nBestHeight > 12500000)
    {
        nSubsidy -= 0.1 * COIN;
        if(nBestHeight > 26785714)
        {
            nSubsidy -= 0.1 * COIN;
            if(nBestHeight > 43452380)
            {
                nSubsidy -= 0.1 * COIN;
                if(nBestHeight > 63452380)
                {
                    nSubsidy -= 0.1 * COIN;
                    if(nBestHeight > 88452380)
                    {
                        nSubsidy -= 0.1 * COIN;
                        if(nBestHeight > 121785713)
                        {
                            nSubsidy -= 0.1 * COIN;
                            if(nBestHeight > 171785713)
                            {
                                nSubsidy -= 0.1 * COIN;
                            }
                        }
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    // block.GetHash() == 00000d1643263c96467f0e6626f829473000fa3ec193ef07fa4327505c69765e
    // block.hashMerkleRoot == 98b42039d2c8c88a02af565c61740af35a63ecbc33a6fb87231de38e003e6716
    // block.nTime = 1491523200
    // block.nNonce = 867526
    //
    // Testnet Genesis hash
    // block.GetHash() == 0000819df66743871d640ae0cecf5ab92ee40ceb0fc0ceadb1ab64073b50d126
    // block.hashMerkleRoot == 98b42039d2c8c88a02af565c61740af35a63ecbc33a6fb87231de38e003e6716
    // block.nTime = 1491523200
    // block.nNonce = 4430
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Octanox starts up | Fri, 07 Apr 2017 00:00:00 GMT";
        CTransaction txNew;
        txNew.nTime = 1491523200; // Fri, 07 Apr 2017 00:00:00 GMT
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1491523200; // Fri, 07 Apr 2017 00:00:00 GMT
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x98b42039d2c8c88a02af565c61740af35a63ecbc33a6fb87231de38e003e6716"));
        block.print();
